...
func (iClient *Client) CreateMeshInstance(_ context.Context, k8sReq *meshes.CreateMeshInstanceRequest) (*meshes.CreateMeshInstanceResponse, error) {
	...
	if err != nil {
		err = errors.Wrapf(err, "unable to create a new kuma client")
		logrus.Error(err)
		return nil, err
	}
...
func (iClient *Client) executeInstall(ctx context.Context, installmTLS bool, arReq *meshes.ApplyRuleRequest) error {
	...
	if arReq.DeleteOp {
		...
		if err := iClient.applyKumaCRDs(ctx, arReq.DeleteOp); err != nil {
			return err
		}
	}
	yamlFileContents, err := iClient.getLatestKumaYAML(installmTLS)
	if err != nil {
		return err
	}
...
func (iClient *Client) ApplyOperation(ctx context.Context, arReq *meshes.ApplyRuleRequest) (*meshes.ApplyRuleResponse, error) {
	...
	switch arReq.OpName {
	case installmTLSKumaCommand:
		installWithmTLS = true
		fallthrough
	case installKumaCommand:
		go func() {
			opName1 := "deploying"
			if arReq.DeleteOp {
				opName1 = "removing"
			...
			if err := iClient.executeInstall(ctx, installWithmTLS, arReq); err != nil {
				iClient.eventChan <- &meshes.EventsResponse{
					...
					Summary:     fmt.Sprintf("Error while %s Kuma", opName1),
					Details:     err.Error(),
				}
				return
			}
			...
			iClient.eventChan <- &meshes.EventsResponse{
				...
				Summary:     fmt.Sprintf("Kuma %s successfully", opName),
				Details:     fmt.Sprintf("The latest version of Kuma is now %s.", opName),
			}
			return
		}()
	...
	case installBookInfoCommand:
		go func() {
			...
			iClient.eventChan <- &meshes.EventsResponse{
				...
				Details:     fmt.Sprintf("The Kuma canonical Book Info app is now %s.", opName),
			}
			return
		}()
		return &meshes.ApplyRuleResponse{
